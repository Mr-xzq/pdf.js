<template>
  <div class="page">
    <van-nav-bar title="Desktop Complex PDF Reader" left-arrow @click-left="$router.back()" />
...
<script>
import { DesktopComplexPdfReader } from "@/components/pdf/index.js";

export default {
  name: "DesktopComplexReaderDemo",
  components: { DesktopComplexPdfReader },
  data() {
  ...
  methods: {
    confirmUrl() {
      ...
      console.log("[DesktopComplexReaderDemo] confirmUrl: ", this.pdfUrl);
    },

    onLoaded({ info }) {
      console.log("[DesktopComplexReaderDemo] 加载完成: ", info);
      // this.$toast.success(`加载完成`);
    },
    onError(err) {
      console.log("[DesktopComplexReaderDemo] 加载失败: ", err);
